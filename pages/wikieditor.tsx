...
import { Classification, WikiSubject } from './../features/wiki/type';
...
export default function WikiEditor(){
    ...
    const { classification, wiki, wikiSubject, isWikiExist } = useTypedSelector((state)=>state.wiki);
    useEffect(()=>{        
    ...
    return(
        <Paper className = {classes.root}>
            <div className = {classes.titleContainer}>
                ...
                <Button onClick = {()=> router.push({pathname : 'wikiboard' , query: { subjectId : wikiSubject?.subjectId , subjectName : wikiSubject?.subjectName}})} variant="contained" color ="primary" className ={classes.titleButton}>{wikiSubject?.subjectName} 게시판으로 이동</Button>
            </div>
            
            <Paper className ={classes.indexContainer} variant="outlined" >
                <h2>목차</h2>                    
                    {isWikiExist ? classification?.map(item => 
                        <div className = {item.groupId.length > 1 ? (item.groupId.length > 3 ? classes.indexSubTitle2 : classes.indexSubTitle) : classes.indexTitle}>                            
                            <Button onClick={() => onClickIndex(item.groupId)}>{item.groupId}. {item.title}</Button>
                        </div>) : 
                        <div>
                          새롭게 만들어보세요!
                        </div>
                        ...
                        <div style = {{width : '90%'}} className={div.centerFlex}>
                            <Button
                                ...
                                onMouseLeave={handlePopoverClose}><AddCircle fontSize="large"/>
                            </Button>
                        </div>                            
            </Paper>
            ...
            {/* <Button className={classes.submitButton} onClick={()=> submit()}>수정 완료</Button> */}
                <WikiAddEditor wikiId={wiki?.wikiId} open={open} handleModalClose ={()=> handleModalClose()}/>
        </Paper>
